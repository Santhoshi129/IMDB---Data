...
FROM movie;
-- Country, worldwide_gross_income, languages and production have NULL values
...
-- Fimding unique genres using DISTINCT keyword
...
-- Using LIMIT clause to display only the genre with highest number of movies produce 
SELECT     genre,
...
-- 4265 Drama movies were produced intotal and are the highest among all genres 
...
-- Using the result of CTE , we find the count of movies which belongs to only one genre
...
-- Finding the average duration of movies by groupig the genres that movie belongs to
SELECT     genre,
           Round(Avng(duration),2) AS avg_duration
FROM       movie                   AS m
INNER JOIN genre                   AS g
ON         g.movie_id = m.id
GROUP BY   genre
...
--  Action genre has the highest duration of movies by grouping the genres that movie belongs to
...
-- CTE : Finds the rank of each genre based on no of movies in each genre
-- Select query displays the genre rank and the number of movies belonging to the same genre
...
-- FInding the number of movies based on median rating and sorting
...
-- CTE : Findibg the rank of production company based on movie count
-- Quering the CTE to find the production company with rank=1
WITH production_company_hit_movie_summary
     AS(SELECT  prodcution_company,
                Count(movie_id)                     AS MOVIE_COUNT,
                Rank()
                  OVER(
...
-- Dream Warrior Pictures and national Theatre Live Production houses has produced the most no of movies
...
-- 2. During March 2017
-- 3. In the USA 
-- 4. Movies had more than 1000 votes
...
-- Query to find:
-- 1. number of movies of each genre that start with word 'The' (LIKE operator is used for pattern matching)
...
-- There are 8 movies which began with "The" in their title.
...
-- BETWEEN operator is used to find the movies released between 1 April 2018 and April 2019
...
-- Two approchs - one is searched based on language and the other by country column

-- Approch 1: By language column

-- Coumpute the total number of votes for german and italian movies
SELECT languages,
...
-- Query to check if german votes > Italian votes using SELECT IF
...
-- Height, date_of_birth, known_for_movies columns contains NULLS
...
-- CTE : Computes the top 3 genres using average rating > 8 condition and highest movies
-- Using the top genres derived from the CTE, the directors are followed
